...
class MidtermService
{
    ...
    public function cap_table()
    {
        ...
        foreach ($initialData as $row) {
            ...
            $share_class = $this->dao->get_share_class($share_class_id);
            $share_class_category = $this->dao->get_share_class_category_id($share_class_category_id);
            ...
            $result[$share_class_id]['categories'][$share_class_category_id]['investors'][$investor_id]['investor'] = $investor['first_name'] . ' ' . $investor['last_name'];
            $result[$share_class_id]['categories'][$share_class_category_id]['investors'][$investor_id]['diluted_shares'] = $row['diluted_shares'];
            // $result['categories'] = $share_class_category['description'];
            // $result['investor'] = $investor['first_name'];
        }
